...
pub(crate) fn reflect_2d(
    ...
    match condition {
        ...
        BoundaryCondition::PrecalculatedReflectionCoefficient => {
            return Err("precalculated internal reflection coefficients are not supported");
        }
    }
...
fn grain_properties(environment: &EnvironmentCase, grain_size_phi: f64) -> HalfSpaceProperties {
    ...
    let density_g_cm3 = environment
        .sound_speed
        .points
        .last()
        .map_or(1.0, |point| point.density_g_cm3);
    HalfSpaceProperties {
        compressional_speed: material_complex_speed(
            0.0,
            real_speed,
        ...
        // v2023.5 assigns the final water-profile density here rather than the
        // handbook density-ratio polynomial. Preserve that behavior.
        density_g_cm3,
    }
}
...
fn apply_half_space_reflection(
    ...
    if reflection.norm() < 1.0e-5 {
        reflected.amplitude = 0.0;
    } else {
        reflected.amplitude *= reflection.norm();
        reflected.phase_radians += reflection.arg();
        if beam_shift {
            apply_beam_shift(
                reflected,
                incident,
...
#[cfg(test)]
mod tests {
    use crate::model::{ReflectionCoefficientPoint, ReflectionCoefficientTable};

    use super::interpolate_reflection;
